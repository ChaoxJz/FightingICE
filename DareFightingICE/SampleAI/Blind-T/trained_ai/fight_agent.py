...
from encoder import RawEncoder, FFTEncoder, MelSpecEncoder, SampleEncoder, TransformerEncoder
import random
...
class SoundAgent:
    def __init__(self, gateway, **kwargs):
        ...
        self.actions = "STAND_D_DB_BA","STAND_D_DF_FC",\
                       "BACK_STEP","FOR_JUMP","CROUCH_GUARD","AIR_GUARD","STAND_GUARD",\
                       "CROUCH_B","CROUCH_FB","AIR_DB","AIR_FA"
...
#             return
        ...
        if self.just_inited:
            ...
            action_idx = np.random.choice(11, 1, replace=False)[0]
        else:
            if obs is None:
        ...
        '''
        com_rand=random.randint(0,100)
        if com_rand<12:
            self.cc.commandCall("STAND_D_DF_FC")
        elif com_rand>95:
            self.cc.commandCall("STAND_D_DF_FB")
        else:
            self.cc.commandCall(self.actions[int(action_idx)])
        '''
