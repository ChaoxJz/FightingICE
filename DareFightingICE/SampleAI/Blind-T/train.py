...
def process_game_agent_data(actor_, critic_, data_collect_helper, recurrent):
    ...
    for r in tqdm.trange(number_round):
        ...
        for i in range(1, min(len(round_data) - 1, len(round_actions))):
            ...
            terminal = torch.tensor(done, device=GATHER_DEVICE).float()
            # transformed_reward = normalize_reward(reward)
            transformed_reward = reward
            round_true_reward_list.append(torch.tensor(reward, device=GATHER_DEVICE).float())
            round_reward_list.append(torch.tensor(transformed_reward, device=GATHER_DEVICE).float())
...
def collect_trajectories(actor, critic, port, game_num, p2, rnn, n_frame):
    ...
    while error:
        ...
        try:
            ...
            agent = SoundAgent(gateway, actor=actor, critic=critic, collect_data_helper=collect_data_helper, logger=logger, n_frame=n_frame, rnn=rnn, device=GATHER_DEVICE)
...
def calc_discounted_return(rewards, discount, final_value):
    ...
    """
    seq_len = len(rewards)
    discounted_returns = torch.zeros(seq_len).to(rewards.device)
...
def compute_advantages(rewards, values, discount, gae_lambda):
    ...
    seq_len = len(rewards)
    advs = torch.zeros(seq_len + 1).to(rewards.device)
...
def pad_and_compute_returns(trajectory_episodes, len_episodes):
    ...
    for i in range(episode_count):
        single_padding = torch.zeros(episode_max_length - len_episodes[i], device=GATHER_DEVICE)
        for key, value in trajectory_episodes.items():
            if value[i].ndim > 2:
                padding = torch.zeros(episode_max_length - len_episodes[i], value[0].shape[1], value[0].shape[2],
                                      dtype=value[i].dtype, device=value[i].device)
            elif value[i].ndim > 1:
                padding = torch.zeros(episode_max_length - len_episodes[i], value[0].shape[1], dtype=value[i].dtype, device=value[i].device)
            else:
                padding = torch.zeros(episode_max_length - len_episodes[i], dtype=value[i].dtype, device=value[i].device)
            try:
...
def train_model(actor, critic, actor_optimizer, critic_optimizer, iteration, port, encoder_name, experiment_id, p2, recurrent, n_frame):
    ...
    while iteration < TRAINING_ITERATION:
        ...
        logger.info(f'clear cuda memory')
        ...
        stddev_reward = np.std(trajectories["true_rewards"].sum(axis=1).cpu().numpy())
        ...
        logger.info(f'Saved parameters')
